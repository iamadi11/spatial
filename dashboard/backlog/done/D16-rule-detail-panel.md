...
1. Added `useState(false)` for `expanded` toggle — local state, no props change
2. Added `SEVERITY_EXPLANATION` record mapping `warning`/`error` to human-readable strings
3. Toggle button with `aria-expanded` / `aria-label` ("Show details" / "Hide details")
4. Expandable panel rendered conditionally: severity explanation `<p>` + `<Link to="/analyze">Try in Playground →</Link>`
5. All existing props/rendering unchanged — purely additive

Files changed: `dashboard/src/components/RuleCard.tsx` (1 file)
...
| QA Validation | PASS | 2 happy paths, 2 edge cases, 1 failure, 1 unknown — all in test file |
| Dev Validation | PASS | Pure React component, strict types, no `any`, ARIA labels present |
| Test Coverage | PASS | 6/6 tests pass; 141/141 full dashboard suite passes |

Overall: PASS

## QA Test Plan

**Happy path 1**: `RuleCard` renders a "Show details" button with `aria-expanded="false"` by default
**Happy path 2**: clicking the button toggles to expanded state — shows severity explanation and playground link; button text changes to "Hide details"; `aria-expanded="true"`
**Edge case 1**: clicking again collapses — panel hidden, `aria-expanded` back to false
**Edge case 2**: expanded panel shows "Try in Playground" link pointing to `/analyze`
**Failure case**: severity explanation reflects the rule's severity — "warning = investigate" for warning rules
**Unknown/deterministic**: `RuleCard` always renders the toggle button regardless of rule metadata values
